...
// Same slug convention used by the homepage brand icons
const slugifyBrand = (brand: string) => brand.toLowerCase().replace(/ /g, '-');
...
export default function BrandListingPage({
  ...
  useEffect(() => {
    const loadProducts = async () => {
      ...
      // Recover the brand's real casing from the products; fall back to de-slugifying
      const match = (products || []).find((p: any) => p.brand && slugifyBrand(p.brand) === slug);
      setBrandName(match?.brand || slug.split('-').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' '));
      setIsLoading(false);
    };
    loadProducts();
  }, [params]);
  ...
  const filteredProducts = allProducts.filter((p: any) => {
    ...
    return matchesBrand && matchesSearch && matchesDevice && matchesCategory && matchesPrice;
  });
    ...
    <div className="min-h-screen bg-white text-black font-sans">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-52 sm:pt-40 pb-20 flex flex-col md:flex-row gap-10">
        ...
        <aside className="w-full md:w-64 flex-shrink-0 border-r-2 border-black/5 pr-0 md:pr-8 flex flex-col gap-10 text-left">
          ...
          <div>
            <h4 className="text-[9px] font-black uppercase text-zinc-400 mb-4 tracking-widest">Protection Type</h4>
            <div className="flex flex-col gap-3">
              {[
                { id: "Tempered Glass", label: "Tempered Glass" },
                { id: "Camera Guard", label: "Camera Guard" },
                { id: "Back ScreenGuard", label: "Back ScreenGuard" },
                ...
                <label key={item.id} className="flex items-center gap-3 cursor-pointer group">
                  <input
                    ...
                    className="w-4 h-4 border-2 border-black rounded-none accent-black cursor-pointer"
                  />
                  <span className={`text-xs font-bold transition-all uppercase ${selectedCategories.includes(item.id) ? 'text-black font-black' : 'text-zinc-500 group-hover:text-black'}`}>
                    {item.label}
                  </span>
                </label>
              ))}
          ...
          <div className="flex flex-col md:flex-row justify-between items-end mb-10 border-b-4 border-black pb-4 gap-4">
            <div>
              <h2 className="text-3xl font-black uppercase italic tracking-tighter text-black">
                {brandName} <span className="text-[#fbea27] drop-shadow-[2px_2px_0px_rgba(0,0,0,1)]">Vault</span>
              </h2>
              {!isLoading && (
