...
import { X, Trash2, Package, ShoppingBag } from 'lucide-react';
...
export interface ShopFilters {
  category?: string; // e.g. "tempered-glass" -> product.category
  device?: string;   // e.g. "Mobile" -> product.deviceType
  type?: string;     // e.g. "UV Glass" -> product.subCategory
  finish?: string;   // e.g. "Matte Finish" -> product.types[].name
  model?: string;    // e.g. "iPhone 15 Pro" -> product.model
  tag?: string;      // e.g. "best seller" -> product.tag
}
...
// Tolerant matcher: menu wording and stored values differ slightly
// ("UV Glass" menu vs "UV" product, "Matte Finish" vs "Matte"), so we
// normalize both sides and accept containment either way.
...
export default function ShopClient({ filters }: { filters: ShopFilters }) {
  ...
  const filteredProducts = allProducts.filter((p: any) => {
    if (filters.category && !fuzzyMatch(p.category, filters.category)) return false;
    if (filters.device && !fuzzyMatch(p.deviceType, filters.device)) return false;
    if (filters.type && !fuzzyMatch(p.subCategory, filters.type)) return false;
  ...
  // Human-readable header built from the active filters
  const isBestSellers = fuzzyMatch(filters.tag, 'best seller');
  const titleParts = [
    filters.category ? (CATEGORY_LABELS[filters.category] || filters.category) : null,
    filters.model || null,
    filters.device || null,
    filters.type || null,
    filters.finish || null,
    isBestSellers ? 'Best Sellers' : (filters.tag || null),
  ].filter(Boolean) as string[];
  const pageTitle = titleParts[0] || 'All Products';
  const filterChips = titleParts.slice(1);
      ...
      <div className="max-w-7xl mx-auto px-6 pt-52 sm:pt-40 pb-20 flex flex-col md:flex-row gap-10">
        ...
        <aside className="w-full md:w-64 flex-shrink-0 border-r-2 border-black/5 pr-0 md:pr-8 flex flex-col gap-10 text-left">
          ...
          {filterChips.length > 0 && (
            <div>
              <h4 className="text-[9px] font-black uppercase text-zinc-400 mb-4 tracking-widest">Active Filters</h4>
              <div className="flex flex-wrap gap-2">
                {filterChips.map((chip) => (
                  <span key={chip} style={{ backgroundColor: BRAND_YELLOW }} className="px-3 py-1.5 text-[9px] font-black uppercase tracking-widest text-black border-2 border-black">
                    {chip}
                  </span>
                ))}
              </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-end mb-10 border-b-4 border-black pb-4 gap-4">
            <div>
              <h2 className="text-3xl font-black uppercase italic tracking-tighter text-black">
                {pageTitle} <span className="text-[#fbea27] drop-shadow-[2px_2px_0px_rgba(0,0,0,1)]">Vault</span>
              </h2>
              {!isLoading && (
