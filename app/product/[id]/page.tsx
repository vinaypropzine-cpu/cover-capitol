...
export default function ProductDetail() {
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] text-black font-sans">
      ...
      {loading ? (
        /* HIGH-FIDELITY SKELETON LAYOUT */
        <main className="max-w-7xl mx-auto px-4 py-8 lg:py-12 animate-pulse" style={{ marginTop: '96px' }}>
          <div className="grid lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-7 flex flex-col-reverse lg:flex-row gap-4">
              <div className="flex lg:flex-col gap-3 overflow-x-auto lg:overflow-x-visible no-scrollbar">
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className="w-16 h-16 lg:w-20 lg:h-20 flex-shrink-0 bg-zinc-200 rounded-lg"></div>
                ))}
              </div>
              <div className="flex-1 bg-zinc-200 rounded-2xl aspect-square"></div>
            </div>

            {/* Info Skeleton */}
            <div className="lg:col-span-5 flex flex-col justify-between">
              <div>
                <div className="h-6 w-24 bg-zinc-200 rounded-md mb-4"></div>
                <div className="h-10 w-3/4 bg-zinc-300 rounded-md mb-4"></div>
                <div className="space-y-2 mb-6">
                  <div className="h-4 w-full bg-zinc-200 rounded"></div>
                  <div className="h-4 w-5/6 bg-zinc-200 rounded"></div>
                  <div className="h-4 w-4/6 bg-zinc-200 rounded"></div>
                </div>
                <div className="h-10 w-32 bg-zinc-300 rounded-md mb-8"></div>
                
                <div className="flex gap-3 mb-6">
                  <div className="h-12 flex-1 bg-zinc-200 rounded-xl"></div>
                  <div className="h-12 flex-1 bg-zinc-200 rounded-xl"></div>
                  <div className="h-12 flex-1 bg-zinc-200 rounded-xl"></div>
                </div>
                
                <div className="h-20 w-full bg-zinc-200 rounded-sm mb-6"></div>
              </div>

              <div className="flex flex-col gap-3 mt-auto">
                <div className="h-14 w-full bg-zinc-300 rounded-xl"></div>
                <div className="h-14 w-full bg-zinc-200 rounded-xl"></div>
              </div>
            </div>
          </div>
        </main>
      ) : !product ? (
        /* PRODUCT MISSING LAYOUT */
        <main className="max-w-7xl mx-auto px-4 py-32 text-center" style={{ marginTop: '96px' }}>
          <ShieldAlert size={64} className="mx-auto text-zinc-300 mb-6" />
          <h1 className="font-black uppercase tracking-tighter text-4xl italic text-black">Product Missing</h1>
          <p className="text-zinc-500 font-bold uppercase text-xs mt-2">The requested item is no longer in the vault.</p>
          <Link href="/">
            <button className="mt-8 bg-black text-white px-8 py-3 font-black uppercase text-xs hover:bg-[#fbea27] hover:text-black transition-all">
              Return Home
            </button>
          </Link>
        </main>
      ) : (
        /* NORMAL RENDER: LIVE PRODUCT DATA */
